...
- **Icon Badge** — Your current chat usage % is shown right on the extension icon (green/orange/red).
- **Auto-Refresh** — Data refreshes every 5 minutes in the background; no tabs need to be open.
...
The extension fetches usage data directly from the background service worker using your existing browser session. If a direct fetch is blocked, it falls back to content scripts running on an open tab:
...
- The extension only communicates with `claude.ai` and `platform.claude.com`.
...
| Issue | Solution |
|-------|----------|
| "Not logged in" (Chat) | Open claude.ai and sign in, then hit refresh. |
| "Not logged in" (API) | Open platform.claude.com and sign in, then hit refresh. |
